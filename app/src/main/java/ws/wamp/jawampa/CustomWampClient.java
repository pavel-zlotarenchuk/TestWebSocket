...
public class CustomWampClient {
    CustomWampClient(ClientConfiguration clientConfig)
    {
    ...
    public <T> Observable<T> makeSubscription(String topic, SubscriptionFlags flags,
                                                         final Class<T> eventClass) {
        return this.makeSubscription(topic, flags).map(new Func1<PubSubData, T>() {
            public T call(PubSubData ev) {
                if (eventClass != null && eventClass != Void.class) {
                    if (ev.arguments != null && ev.arguments.size() >= 1) {
                        String result = ev.arguments.toString();
                        return (T) result;
                    } else {
                        throw OnErrorThrowable.from(new ApplicationError("jawampa.error.missing_value"));
                    }
                } else {
                    return null;
                }
...
/*    public <T> Observable<T> makeSubscription(final String topic, SubscriptionFlags flags, final Class<T> eventClass)
    ...
    public Observable<PubSubData> makeSubscription(final String topic, final SubscriptionFlags flags) {
        return Observable.create(new Observable.OnSubscribe<PubSubData>() {
            @Override
    ...
    public Observable<Void> getTerminationObservable() {
        ...
        stateController.statusObservable().subscribe(new Observer<WampClient.State>() {
            @Override
            ...
            @Override
            public void onNext(WampClient.State t) { }
        });
        return termSubject;
    }
